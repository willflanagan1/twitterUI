...
const buildTweets = function (data) {
          data.forEach(tweet => {
               ...
               thisTweetContainer = $("<div tweetID=" + tweetID + " isLiked= " + tweet.isLiked + ">")
               ...
               let tweetChildren = thisTweetContainer.children();
               var children = [].slice.call(tweetChildren);
               let xssScriptCheck = false;
               var verifiedClasses = ['parent-container', 'button-container', 'author', 'tweet-body'];
               children.forEach(child => {
                    ...
                    if (!verifiedClasses.includes(childClass)) {
                         child.remove();
                         if (xssScriptCheck == false) {
                              thisTweetContainer.empty();
                              xssScriptCheck = true;
                              tweet.body = "This tweet is unavailable due to cross-site scripting";
                              thisTweetContainer.append("<p class=author style='font-size: 20px'><strong style='color: rgb(32, 156, 238)'>" + tweet.author + "</strong></p>")
                              thisTweetContainer.append("<p class=tweet-body style='color: red'>" + tweet.body + "</p>")
                         }
                    }
               })
     ...
     $("#post-button").click(handlePostButtonPress);
     $("#clear-button").click(handleClearButtonPress);
};
